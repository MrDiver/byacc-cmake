...
void create_file_names(void) {
  ...
  _mktemp_s(union_file_name, i);
  _mktemp_s(output_file_name, i);
#endif
  ...
  if (dflag) {
    if (jflag) {
      ...
      defines_file_name =
          MALLOC(jclass_len + strlen(JAVA_INTERFACE_SUFFIX JAVA_OUTPUT_SUFFIX) +
                 1); /*rwj for 'Tokens.java\0' */
      if (defines_file_name == 0)
        no_space();
      strcpy(defines_file_name, jclass_name);
      strcpy(defines_file_name + jclass_len,
             JAVA_INTERFACE_SUFFIX JAVA_OUTPUT_SUFFIX);
      if (jimplement_name && strlen(jimplement_name) > 0) {
        char *impl_name = MALLOC(strlen(jimplement_name) + jclass_len +
                                 strlen(JAVA_INTERFACE_SUFFIX) + 2);
        ...
        jimplement_name =
            MALLOC(jclass_len + strlen(JAVA_INTERFACE_SUFFIX) + 1);
        if (jimplement_name == 0)
          no_space();
...
void write_temporary_output(void) {
  ...
  if (jflag) /*rwj*/
  {
    ...
    output_file_name =
        MALLOC(len + strlen(JAVA_OUTPUT_SUFFIX) + 1); /*rwj for '.java\0' */
    if (output_file_name == 0)
      no_space();
